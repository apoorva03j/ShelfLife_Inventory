...
const GrievanceView = () => {
  ...
  useEffect(() => {
    const fetchGrievances = async () => {
      try {
        const response = await axios.get('http://localhost:8080/grievance-view');
  ...
  const handleResponseSubmit = async () => {
    ...
    try {
      await axios.put(`http://localhost:8080/grievances/${selectedGID}`, updatedGrievance);
  ...
  const handleResolvedToggle = (gId) => {
    ...
    axios.put(`http://localhost:8080/grievances/${gId}`, updatedGrievance)
      .then(() => {
        setGrievances(grievances.map(grievance =>
