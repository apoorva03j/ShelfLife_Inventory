...
const EmployeeView = () => {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/get-employees/${cid}`);
  ...
  const handleDeleteClick = async (uid) => {
    try {
      await axios.delete(`http://localhost:8080/delete-employee/${uid}`);
  ...
  const handleSaveClick = async () => {
    try {
      await axios.put(`http://localhost:8080/update-employee/${editEmployee.uid}`, editEmployee);
